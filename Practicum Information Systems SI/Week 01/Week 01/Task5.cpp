...
void sortGroup(Group& group) {
	...
	for (size_t i = 0; i < group.groupSize - 1; i++)
	{
		int minElInd = i;
		for (size_t j = i+1; j < group.groupSize; j++){
			if (group.students[i].averageGrade < group.students[j].averageGrade)
				minElInd = j;
		}
		if (minElInd != i)
			std::swap(group.students[i], group.students[minElInd]);
	}
}

void sortedScholarshiped(const Group& group, double minForScholarship) {

	size_t studentsWithScholarship = studentsToGetScholarship(group, minForScholarship);
	Group newGroup;
	newGroup.groupSize = studentsWithScholarship;

	unsigned index = 0;

	for (size_t i = 0; i < group.groupSize; i++){
		if (group.students[i].averageGrade >= minForScholarship)
			newGroup.students[index++] = group.students[i];
	}
...
int main() {
	...
	sortedScholarshiped(groupOfStudents, minForScholarship);
